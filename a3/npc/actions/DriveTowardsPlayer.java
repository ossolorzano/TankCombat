...
public class DriveTowardsPlayer extends BTAction{
	...
	public DriveTowardsPlayer(Tank player, Tank npc, TerrainBlock terrain){
		this.player = player;
		this.npc = npc;
	...
	protected BTStatus update(float time) {
		...
		if(theta<thetaOffset&&theta>-thetaOffset){
			moveForward.performAction(time, null);
		}
		else{
